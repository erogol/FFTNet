...
def convert_file(path):
    ...
    quant = quant.clip(0, 2 ** CONFIG.bits - 1)
...
def process_wav(wav_path):
    ...
    assert x.max() < 2 ** CONFIG.bits, wav_path
    assert x.min() >= 0
...
if __name__ == "__main__":
    ...
    x = ap.dequantize(x)
    librosa.output.write_wav(OUT_PATH + "test_converted_audio.wav", x, sr=CONFIG.sample_rate)
